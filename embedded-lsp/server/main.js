...
const PinTracker = require('./pin-tracker');
...
class OrbitEmbeddedLanguageServer {
    constructor() {
        ...
        this.pinTracker = new PinTracker();
    ...
    async handleDocumentChange(change) {
        ...
        // Track pin usage and send to client
        const pinUsages = this.pinTracker.analyzePinUsage(change.document);
        const pinMap = this.pinTracker.buildPinMap(pinUsages);
        
        // Send pin map to client as custom notification
        this.connection.sendNotification('sentinel/pinMap', {
            uri: change.document.uri,
            pinMap
        });

        // Live memory analysis - estimate RAM/Flash as user types
        const memoryAnalysis = this.memoryAnalyzer.analyzeMemory(change.document);
        
        // Send memory analysis to client
        this.connection.sendNotification('sentinel/memoryAnalysis', {
            uri: change.document.uri,
    ...
    async handleDocumentOpen(event) {
        ...
        // Analyze document immediately on open
        const diagnostics = await this.diagnosticProvider.validateDocument(
            event.document, 
            this.settings
        );
        ...
        // Track pin usage and send to client
        const pinUsages = this.pinTracker.analyzePinUsage(event.document);
        const pinMap = this.pinTracker.buildPinMap(pinUsages);
        ...
        const memoryAnalysis = this.memoryAnalyzer.analyzeMemory(event.document);
    ...
    async handleConfigChange(change) {
        if (change.settings && change.settings.orbitIDE) {
